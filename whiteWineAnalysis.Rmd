...
# global config to normalize the plots and help to avoid repetitive configs
```
...
# Here we add a new variable with the data of quality grupped in 3 different chunks
...
wines$classification <- ordered(wines$classification, levels = c("bad", "medium", "good"))
# Now I have converted it to a factor and order it so it is reasonable
```

```{r echo=FALSE, Useful_functions}
#Creation of one function so the creation of a basic histogram can be made easily and avoiding repetitions, with only sending the x variable and the binwidth
create_plot <- function(variable, binwidth = 0.01) {
  return(ggplot(aes_string(x = variable), data = wines) + 
           geom_histogram(binwidth = binwidth))}
# This function helps to create scatterplots with an especific set of colors given the x, y and color passed through parameters 
create_plot_compare <- function(variable_x, variable_y, color) {
  return(ggplot(aes_string(x = variable_x, y = variable_y, color = color), data = wines) + 
           geom_point() +
           scale_color_brewer(palette = "GnBu"))}

correlation <- function(variable1, variable2, n){
  sxy <- sum(variable1 * variable2) - (sum(variable1) * sum(variable2)) / n 
  sxx <- sum(variable1^2) - sum(variable1)^2/n
  syy <- sum(variable2^2) - sum(variable2)^2/n
...
Here we see the distribution of the pH values across our dataset. The preferred value of our data seems to be between 3.0 a 3.25. Given the significance of the pH values it is interesting to see that are some values that are getting close to 4. which mean that those wines are less acid. The same can be said for those values closing in on 0, which will mean that they are extremely acid. One thing that could be interesting is to see if there is any correlation between the acidity of a wine and its quality.

```{r Univariate_Plots_5}
create_plot('volatile.acidity', 0.01)
```

Looking at the volatile acidity distribution we see that almost the whole dataset converge around the 0.3 value. It looks that the values are very rarely above 1. It is interesting to see that the values seem to constantly decrease towards higher values. Looking at this plot makes me wonder about how come there are wines with a volatile acidity that its more than three times higher than the average. Could this affect the quality, I will look into it in the next parts.  

```{r Univariate_Plots_6}
create_plot('total.sulfur.dioxide', 10)
```

In this case the plot seems to be a little bit symmetrical converging around 100 and 150. Taking into account that there seem to be some outliers that have values above 300. In this case we call them outliers because as you can see they seem to fall apart from one another and visually we cant count more than 4 or 5 wines. This can indicate the existence of wines with really different values that can hurt possible predictions or future plots.
...
p1 <- create_plot_compare('fixed.acidity', 'X', "classification")
p2 <- create_plot_compare('fixed.acidity', 'X', "quality")
...
p1 <- create_plot_compare('volatile.acidity', 'X', "classification")
p2 <- create_plot_compare('volatile.acidity', 'X', "quality")
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('citric.acid', 'X', "classification") +
  coord_cartesian(xlim = c(0,quantile(wines$citric.acid, 0.95)))
p2 <- create_plot_compare('citric.acid', 'X', "quality") +
  coord_cartesian(xlim = c(0,quantile(wines$citric.acid, 0.95)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('residual.sugar', 'X', "classification") +
  coord_cartesian(xlim = c(0,quantile(wines$residual.sugar, 0.95)))
p2 <- create_plot_compare('residual.sugar', 'X', "quality") +
  coord_cartesian(xlim = c(0,quantile(wines$residual.sugar, 0.95)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('chlorides', 'X', "classification") +
  coord_cartesian(xlim = c(0,quantile(wines$chlorides, 0.95)))
p2 <- create_plot_compare('chlorides', 'X', "quality") +
  coord_cartesian(xlim = c(0,quantile(wines$chlorides, 0.95)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('free.sulfur.dioxide', 'X', "classification") +
  coord_cartesian(xlim = c(0,quantile(wines$free.sulfur.dioxide, 0.95)))
p2 <- create_plot_compare('free.sulfur.dioxide', 'X', "quality") +
  coord_cartesian(xlim = c(0,quantile(wines$free.sulfur.dioxide, 0.95)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('total.sulfur.dioxide', 'X', "classification") +
  coord_cartesian(xlim = c(0,quantile(wines$total.sulfur.dioxide, 0.98)))
p2 <- create_plot_compare('total.sulfur.dioxide', 'X', "quality") +
  coord_cartesian(xlim = c(0,quantile(wines$total.sulfur.dioxide, 0.98)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('density', 'X', "classification") +
  coord_cartesian(xlim = c(quantile(wines$density, 0.00),quantile(wines$density, 0.95)))
p2 <- create_plot_compare('density', 'X', "quality") +
  coord_cartesian(xlim = c(quantile(wines$density, 0.00),quantile(wines$density, 0.95)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('pH', 'X', "classification") +
  coord_cartesian(xlim = c(quantile(wines$pH, 0.00),quantile(wines$pH, 0.98)))
p2 <- create_plot_compare('pH', 'X', "quality") +
  coord_cartesian(xlim = c(quantile(wines$pH, 0.00),quantile(wines$pH, 0.98)))
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('sulphates', 'X', "classification")
p2 <- create_plot_compare('sulphates', 'X', "quality")
grid.arrange(p1, p2, ncol = 1)
...
p1 <- create_plot_compare('alcohol', 'X', "quality")
p2 <- create_plot_compare('alcohol', 'X', "quality")
grid.arrange(p1, p2, ncol = 1)
...
ggpairs(wines, mapping = aes(color=classification), columns = c("X", 
                                                                "fixed.acidity", 
                                                                "volatile.acidity", 
                                                                "citric.acid", 
                                                                "residual.sugar", 
...
create_plot_compare('density', 'alcohol', "quality")+
...
This plot now using the geom smooth in which we can see the progression of the data. As we can see the line seems to be in correlation with the previous plot showing us the direction of the data. We now from the ggpairs function that the correlation is -0.78, however I have written a function to calculate the correlation so we can check if the correlation given by ggpairs is the Pearson correlation. Also we see this correlation being plotted because we see a decremental segment showing that the relationship between the variables in reversed.

```{r}
correlation(wines$density, wines$alcohol, length(wines$alcohol))
```

As we can see the value outputted is the same as the value given by the ggpairs correlation. This have helped me to find different values to compare based on the correlation values.
...
ggplot(wines, aes(x = free.sulfur.dioxide, y = total.sulfur.dioxide, color = quality)) +
  coord_cartesian(xlim = c(0, quantile(wines$free.sulfur.dioxide, 0.99))) +
  geom_point() 
```
...
Here we have the same plot with a geom smooth so we can see the direction of the data as well as confirming the correlation value of 0.616 which means that they are directly correlated. This explains the look of the plot between 0 and 60 of free sulfur dioxide, because we clearly see a steady increase of both values at the same time.
...
```{r Plot_Two}
create_plot_compare('alcohol', 'X', "classification") +
...
create_plot_compare('density', 'alcohol', "quality")+
  ...
  labs(y="Alcohol (cl/L) ", x="Density (g/cm^3)") +
  coord_cartesian(xlim = c(quantile(wines$density, 0.00),quantile(wines$density, 0.99))) +
  theme(plot.title = element_text(hjust = 0.5))
```

...
Once I have studied all the possible direct relationships with quality, I am going to investigate these plots that seemed promissing.
### 
### Alcohol
...
```{r, message=FALSE}
ggplot(wines[wines$classification == "medium" | wines$classification == "bad",], aes(x = density, y = alcohol, color = classification)) +
  geom_point()+
...
```{r}
lm1 <- lm( as.numeric(quality) ~ alcohol/density, data = wines )
...
lm8 <- update(lm7, ~ . + total.sulfur.dioxide)
lm9 <- update(lm8, ~ . + pH)
